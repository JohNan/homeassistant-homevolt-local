...
def _get_device_value(data, orig_fn, device_idx):
    return orig_fn(data, device_idx)


def _get_device_icon(data, orig_fn, device_idx):
    return orig_fn(data, device_idx)
...
class HomevoltSensorEntityDescription(SensorEntityDescription):
    ...
    sensor_type: str | None = (
        None  # The type of sensor this entity is for (grid, solar, load)
    )
...
class HomevoltSensor(CoordinatorEntity[HomevoltData], SensorEntity):
    ...
    def _update_sensor_data(self, data: HomevoltData) -> None:
        ...
        if self.entity_description.value_fn:
            if self.ems_index is not None:
                ...
                # For aggregated and sensor-specific sensors, just pass the data
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        try:
            ...
            if self.ems_index is not None and (
                not data.ems or len(data.ems) <= self.ems_index
            ):
                _LOGGER.debug(
                ...
                self._attr_native_value = None
                self._attr_extra_state_attributes = {}
            elif self.sensor_index is not None and (
                not data.sensors or len(data.sensors) <= self.sensor_index
            ):
                _LOGGER.debug(
...
def _create_device_specific_sensors(
    coordinator: HomevoltDataUpdateCoordinator,
) -> list[HomevoltSensor]:
...
def _create_sensor_specific_sensors(
    coordinator: HomevoltDataUpdateCoordinator,
) -> list[HomevoltSensor]:
    ...
    if coordinator.data and coordinator.data.sensors:
        available_sensor_types = {
            s.type for s in coordinator.data.sensors if s.available
        }
        for description in SENSOR_DESCRIPTIONS:
            if (
                description.sensor_specific
                and description.sensor_type in available_sensor_types
            ):
                for idx, sensor in enumerate(coordinator.data.sensors):
                    if (
                        ...
                        sensors.append(
                            HomevoltSensor(coordinator, description, sensor_index=idx)
                        )
                        break
    return sensors
...
async def async_setup_entry(
    ...
    sensors.extend(_create_device_specific_sensors(coordinator))
    sensors.extend(_create_sensor_specific_sensors(coordinator))
