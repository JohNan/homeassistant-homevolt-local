            ...
            if f.name in data:
                ...
                if hasattr(f.type, 'from_dict'):
                    ...
                    if isinstance(data[f.name], list):
                        kwargs[f.name] = [f.type.__args__[0].from_dict(item) for item in data[f.name]]
                    else:
                        kwargs[f.name] = f.type.from_dict(data[f.name])
                else:
...
class HomevoltData:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> HomevoltData:
        ...
        return cls(
            ...
            schedules=[ScheduleEntry(**s) for s in data.get("schedules", [])],
        )
...
def _add_from_dict_methods():
    ...
    for dc in dataclasses:
        if dc is not HomevoltData: # HomevoltData has a custom from_dict
